...
class trend{	
	...
	void getMomentum(String hashtag, int S, int L, Timestamp start, Timestamp end,database obj){
		...
		database1 smallWindow = new database1(hashtag,S,start,end,true);
		database1 largeWindow = new database1(hashtag,L,start,end,false);
		System.out.println("************** SMALL VALIDITY = "+smallWindow.getValidity());
		System.out.println("************** LARGE VALIDITY = "+largeWindow.getValidity());
		if(smallWindow.getValidity() == -1 || largeWindow.getValidity() == -1){
			success = false;
		...
		File file1 = new File("word_temp.txt");
		file1.delete();
		//Files.delete("word_temp.txt");
		file1 = new File("word_temp.txt");
		file1.createNewFile();
		while ((strLine = br.readLine()) != null)
		{
			String [] list = strLine.split(" ");
			if(list[0].equalsIgnoreCase(hashtag))
			{
	            try(PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(file1, true)))) {
				    out.println(list[0]+" "+smallOccurence);
				}catch (IOException e) {
		...
		{
			try(PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(file1, true)))) {
				    out.println(hashtag+" "+smallOccurence);
				}catch (IOException e) {
				    //exception handling left as an exercise for the reader
				}
		}
		...
		file.delete();
		//Files.delete("word.txt");
		file1 = new File ("word.txt");
		file1.createNewFile();
		fstream = new FileInputStream("word_temp.txt");
		// Get the object of DataInputStream
	...
	public trend(String hashtag, int numUsers, Timestamp start, Timestamp end, int S, int L){
		System.out.println("HASHTAG = "+hashtag+" numUsers = "+numUsers+" Start = "+start+" End = "+end+" S = "+S+" L = "+L);
